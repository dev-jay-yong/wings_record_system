...
async def access_control(request: Request, call_next):
    ...
    try:
        ...
        if (
            await url_pattern_check(url, path_setting["EXCEPT_PATH_REGEX"])
            or url in path_setting["EXCEPT_PATH_LIST"]
        ):
        ...
        request.state.user = user_helper.get_one_user_by_id(
            token_info["user_id"], is_dict=False
        )
        ...
        error_dict = dict(
            message=error.message, detail=error.detail, status_code=error.status_code
        )
